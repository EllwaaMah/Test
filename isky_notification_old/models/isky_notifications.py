...
class IskyNotification(models.Model):
    _name = 'isky.notification'
    ...
    def send_notification(self):
        ...
        for rec in self.env['isky.notification'].search([('state', '=', 'running')]):
            for line in rec.line_ids:
                ...
                if len(partners) > 0 and records and obj_has_notification_list:
                    for obj_has_notification in obj_has_notification_list:
                        ...
                        rec.message_post(message_type="notification", subtype_xmlid='mt_comment', **post_vars)
